...
def dynprog(lang, s_mat, a, b):
    ...
    out_a, out_b = create_alignment(backtrack, lang, s_mat, ind, a, b)
...
def create_alignment(b_mat, lang, s_mat, start, a, b):
    previous_a = start[0]
    previous_b = start[1]
    output = [[], []]
    while previous_a > 0 and previous_b > 0:
        if b_mat[previous_a][previous_b] == b_mat[previous_a - 1][previous_b - 1] + score(lang, s_mat, a[previous_a],
                                                                                          b[previous_b]):
            previous_a -= 1
            previous_b -= 1
            output[0].insert(0, previous_a)
            output[1].insert(0, previous_b)
        elif b_mat[previous_a][previous_b] == b_mat[previous_a - 1][previous_b] + score(lang, s_mat, a[previous_a],
                                                                                        "-"):
            previous_a -= 1
        elif b_mat[previous_a][previous_b] == b_mat[previous_a][previous_b - 1] + score(lang, s_mat, "-",
                                                                                        b[previous_b]):
            previous_b -= 1
        elif b_mat[previous_a][previous_b] == 0:
            break
    return output
